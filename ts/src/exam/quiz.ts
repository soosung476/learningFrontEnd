...
interface Question {
  question: string;
  choices: string[];
  answer: number; // 정답의 인덱스 (choices 배열의 몇 번째가 정답인지)
}
...
// 2. 문제 데이터 가져오기 ---------------------------------------------
const quizData = async <T>(url: string): Promise<T> => {
...
let questions: Question[] = []; // fetch로 받아온 전체 문제 목록 (5개만 뽑아 쓸 예정)
let currentQuestionIndex: number = 0; // 지금 몇 번째 문제를 보여주고 있는지
let score: number = 0; // 맞춘 문제 개수(혹은 점수)
let selectedAnswer: number | null = null; // 사용자가 고른 보기의 인덱스 (아직 안 골랐으면 null)
let gameState: GameState = { playing: true };

const TOTAL_QUESTIONS = 5; // 문제는 5개만 연속으로 출제

// 4. DOM 요소 가져오기 --------------------------------------------------
const quizNum = document.querySelector<HTMLElement>(
  ".quiz-info #question-number",
)!;
const quizScore = document.querySelector<HTMLElement>(".quiz-info #score")!;
...
const resultMsg = document.querySelector<HTMLElement>("#result-message")!;
...
function handleChoiceClick(chosenIndex: number, btn: HTMLButtonElement): void {
  // 여기에 구현
}
...
const main = async () => {
  const data = await quizData<Question[]>("./question.json");
  console.log(data);
  // 여기에 구현 이어서 작성
};
